...
class Features extends BaseController {
	...
	private $admin_url="features";


	public function get_features_details($slug){
	...
	public function load_page_content($ref){
		...
		$this->load->view('dashboard/dashboard_features');
	...
	public function addDefaultData(){
		$data=array(
			...
			"newspaper_screenshot"=>$this->removeHtmlTags($this->input->post("current_picture"))
		);

		/*check if picture is empty*/
		if ($_FILES['picture']['error']!=4) {
			$file_name=$this->do_upload("assets/images/feature/","picture")['upload_data']['file_name'];
			$data['newspaper_screenshot']=$this->createPictureUrl("assets/images/feature/",$file_name);
		}
