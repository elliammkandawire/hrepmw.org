...
class Headers extends BaseController {
	...
	private $admin_url="headers";


	public function get_header_details($slug){
	...
	public function load_page_content($ref){
		...
		$this->load->view('dashboard/dashboard_headers');
	...
	public function addNew(){
		...
		$data['slug']=$this->removeHtmlTags($this->url($this->input->post("menu_name")).date("yyyyMMddHs"));
	...
	public function addDefaultData(){
		$data=array(
			"menu_name"=>$this->removeHtmlTags($this->input->post("menu_name")),
			"image_url"=>$this->removeHtmlTags($this->input->post("current_picture"))
		);

		/*check if picture is empty*/
		if ($_FILES['picture']['error']!=4) {
			$file_name=$this->do_upload("assets/images/headers/","picture")['upload_data']['file_name'];
			$data['image_url']=$this->createPictureUrl("assets/images/headers/",$file_name);
		}
